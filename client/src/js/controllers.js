...
// signin controller
.controller('SigninFormController', ['$rootScope','$scope', '$http', '$state', 'Employee',
 function($rootScope,$scope, $http, $state, Employee) {
...
/*
KHUSUS SPM CONTROLLER
*/

.controller('SPMprofile', ['$scope', '$http', '$state', 'Employee', function($scope, $http, $state, Employee) {
    Employee.getCurrent(function(cb) {
        $scope.me = cb;
        console.log(cb);
    });
}])

.controller('spmListFunction', ['$scope', '$http', '$state', 'Function', function($scope, $http, $state, Function) {
    Function.find({
        filter: {
            order: 'created DESC',
            limit: 30
    ...
    $scope.addNewFunction = function() {
        ...
        if ($scope.addNew.status == undefined) {
            $scope.addNew.status = false;
        }
        Function.create($scope.addNew, function(cb) {
            Function.find({
                filter: {
                    order: 'created DESC',
                    limit: 30
                ...
                $scope.functions = cb;
                $scope.addNew = {};
                alert("Function " + cb.name + " added");
            });
        });
    }

    $scope.activationTogle = function(data) {
...
.controller('appCtrl', ['$scope', '$http', '$state', '$rootScope', function($scope, $http, $state, $rootScope) {
    //pastikan yang mengakses app sudah loged in (meskipun dari tidak ada blocked request dari server)
   

}])

.controller('InfoDashboard', ['$scope', '$http', '$state', 'Product', 'Customer', '$rootScope', function($scope, $http, $state, Product, Customer, $rootScope) {
   Product.count({
      where: {
        status :1
      }
    }, function(result) {
        $scope.cproduct = result;
     });

    Customer.count({
      
    }, function(result) {
        $scope.ccustomer = result;
     });
...
.controller('testFormCtrl', ['$scope', '$http', '$state', '$rootScope', 'Employee', function($scope, $http, $state, $rootScope, Employee) {
    Employee.getCurrent(function(current) {
